SELECT A.eval_trgt_clss,
       A.be_eval_mem_no,
       (CASE
           WHEN(count(aer.eval_stat_cd) > 0) THEN 'N'
           ELSE
               CASE
                   WHEN aer.eval_stat_cd = 'CC01603' THEN 'Y'
                   ELSE 'N'
               END
       END) AS eval_stat
FROM (SELECT er.eval_rel_no,
    ...
    AND A.eval_sheet_no = aer.eval_sheet_no AND A.ablt_ques_no = aer.ablt_ques_no AND A.eval_plan_no = aer.eval_plan_no
 GROUP BY A.eval_trgt_clss, A.be_eval_mem_no
 ORDER BY A.eval_trgt_clss desc