...
def calc_eval_rslt(eval_plan_no):
    ...
    for result in result_list:
        if result['CC009004'] is not None:
            tot_cc009004_sum += float(result['CC009004'])
            tot_cc009004_cnt += 1

        if result['CC009003'] is not None and result['CC009002'] is not None and result['CC009001'] is not None:
            # 부하평가 비중
            m_point = float(result['CC009001']) * (eval_plan_qs.m_eval_wght / 100)
            tot_cc009001_sum += float(result['CC009001'])
            tot_cc009001_cnt += 1
            # 상사평가 비중
            s_point = float(result['CC009002']) * (eval_plan_qs.s_eval_wght / 100)
            tot_cc009002_sum += float(result['CC009002'])
            tot_cc009002_cnt += 1
            # 동료평가 비중
            j_point = float(result['CC009003']) * (eval_plan_qs.j_eval_wght / 100)
            tot_cc009003_sum += float(result['CC009003'])
            tot_cc009003_cnt += 1
